...
func main() {
	...
	c, closeFunc, err := gocache.NewGoCache(cfg.Redis().Address(), cfg.Redis().Eviction())
	if err != nil {
		panic(err)
	}
	...
	checkerHandler := checker.NewHandler(aclStore, c, cache.NewProtoAttributeSerializer(), cache.NewCustomCheckPermissionSerializer())
	...
	err = syncerasync.NewSyncerAsyncApi(subscriber, "sync", "oort", syncerpb.NewProtoSyncMessageSerializer(), syncerHandler)
	if err != nil {
		panic(err)
	}
